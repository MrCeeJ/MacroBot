...
public class BuildManager {
    ...
    private boolean buildExtractor(Base base) {
        List<Point2d> gasPositions = base.getGases().stream().map(u -> u.unit().getPosition().toPoint2d()).collect(Collectors.toList());
        ...
        if (gasPositions.size() > extractorPositions.size()) {
            for (Point2d position : gasPositions) {
                if (!extractorPositions.contains(position)) {
